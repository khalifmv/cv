import {Curtains, Plane, Vec2, Vec3} from 'https://cdn.jsdelivr.net/npm/curtainsjs@8.1.2/src/index.mjs';
...
window.addEventListener("load", () => {
    ...
    curtains.onRender(() => {
        ...
        scrollEffect = curtains.lerp(scrollEffect, 0, 0.095);
    ...
    const vs = `
        ...
        void main() {
            ...
            vertexPosition.x += sin((vertexPosition.y / 1.5 + 1.0) * 3.141592) * (sin(uScrollEffect / 820.0));
    ...
    function handlePlanes(index) {
        ...
        plane.onReady(() => {
            ...
            // apply the rotation
            // plane.rotation.z = Math.abs(scrollEffect) / 650;

            // scale plane and its texture
            plane.scale.y = 1 + Math.abs(scrollEffect) / 180;
            plane.scale.x = 1 + Math.abs(scrollEffect) / 180;
            plane.textures[0].scale.y = 1 + Math.abs(scrollEffect) / 150;
            plane.textures[0].scale.x = 1 + Math.abs(scrollEffect) / 200;
    ...
    function applyPlanesParallax(index) {
        ...
        planes[index].relativeTranslation.y = parallaxEffect * sceneBoundingRect.height / 8;
